...
function ticketUser(u: SupportTicket['user']) {
  return typeof u === 'string' ? null : u;
}
...
export default function AdminSupportPage() {
  ...
  const visibleTickets = tickets.filter((tk) => filter === 'all' || tk.status === filter);
  ...
  async function handleToggleStatus() {
    ...
    try {
      const res = await api.patch<ApiResponse<{ ticket: SupportTicket }>>(`/support/${active._id}/status`, {
        status: nextStatus,
      });
      setActive(res.data.data.ticket);
      setTickets((prev) => prev.map((tk) => (tk._id === active._id ? res.data.data.ticket : tk)));
      toast.success(nextStatus === 'closed' ? t('support.markedClosed') : t('support.markedOpen'));
    } catch (error) {
      toast.error(getApiErrorMessage(error));
    } finally {
          ...
          {(['open', 'closed', 'all'] as const).map((f) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 font-medium transition-colors ${
                filter === f ? 'bg-accent-500 text-white' : 'text-ink-soft hover:bg-line-soft'
              }`}
            >
              {f === 'open' ? t('support.statusOpen') : f === 'closed' ? t('support.statusClosed') : t('support.filterAll')}
            </button>
          ))}
        </div>
      </div>
          ...
          {loading ? (
            ...
            <ul className="space-y-2">
              {visibleTickets.map((tk) => {
                const owner = ticketUser(tk.user);
                return (
                  <li key={tk._id}>
                    <button
                    ...
                    >
                      ...
                      <p className="mt-0.5 truncate text-xs text-ink-faint">{owner?.name} · {owner?.email}</p>
              ...
              <div className="flex items-center justify-between border-b border-line px-5 py-4">
                <div>
                  ...
                  <p className="text-xs text-ink-faint">
                    {ticketUser(active.user)?.name} · {ticketUser(active.user)?.email}
                  </p>
                </div>
                <button
                ...
                >
                  {active.status === 'open' ? <CheckCircle2 size={14} /> : <RotateCcw size={14} />}
                  {active.status === 'open' ? t('support.markClosed') : t('support.reopen')}
                </button>
              </div>
